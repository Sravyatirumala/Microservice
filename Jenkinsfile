pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = 'sravyatirumala/emailservice:latest'
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'Docker-creds', toolName: 'docker') {
                        sh "docker build --no-cache -t sravyatirumala/emailservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'Docker-creds', toolName: 'docker') {
                        sh "docker push sravyatirumala/emailservice:latest"
                    }
                }
            }
        }
